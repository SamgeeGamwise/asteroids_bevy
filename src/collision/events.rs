use crate::asteroid::components::{Asteroid, AsteroidSize};
use crate::bullet::Bullet;
use crate::player::Player;
use crate::settings::{AsteroidSettings, BulletSettings, PlayerSettings};
use bevy::prelude::*;
...
pub fn detect_collisions(
    ...
    for i in 0..asteroids.len() {
        for j in (i + 1)..asteroids.len() {
            ...
            if dist
                < asteroid_radius(&asteroid_settings, asteroid_a.size)
                    + asteroid_radius(&asteroid_settings, asteroid_b.size)
            {
        ...
        for j in 0..bullets.len() {
            ...
            let dist = asteroid_transform
                .translation
                .distance(bullet_transform.translation);

            if dist
                < bullet_settings.hitbox_radius + asteroid_radius(&asteroid_settings, asteroid.size)
            {
                commands.trigger(BulletAsteroidCollision {
    ...
    if let Ok((player_entity, player_transform)) = player_query.single() {
        for (asteroid_entity, asteroid_transform, asteroid) in &asteroids {
            ...
            if dist
                < player_settings.hitbox_radius + asteroid_radius(&asteroid_settings, asteroid.size)
            {
